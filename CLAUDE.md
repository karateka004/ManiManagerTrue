...
- **Дозаполнение карточек:** `backfillCards(env, BACKFILL_SLICE=12)` — `POST /admin/stats {backfill:true}`
  и порция каждую ночь в `nightlyAnalytics`. Курсор в `admin:backfill-cursor`. Порция маленькая
  намеренно — потолок подзапросов Worker; **UI сам крутит порции в цикле** (`runBackfill`, до 40 кругов),
  поэтому нажатие одно. Пока покрытие неполное, под верхними карточками висит плашка со списком
  занижённых чисел, а «записывают операции» показывает прочерк вместо нуля.
