...
- **Дозаполнение карточек:** `backfillCards(env, BACKFILL_SLICE=12)` — кнопка «Дозаполнить карточки»
  (`POST /admin/stats {backfill:true}`) и порция каждую ночь в `nightlyAnalytics`. Курсор в
  `admin:backfill-cursor`. Порция маленькая намеренно — потолок подзапросов Worker.
