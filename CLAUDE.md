...
**Мини-игра «Сад» (закрытый тест):** дерево, растущее от активности. Ключевой принцип —
игра НЕ вводит новых сущностей: рост считается из XP, полив = существующий `claimDailyStreak`,
улучшения тратят обычные монеты, редкие элементы открываются по уже считающимся метрикам.
- `src/lib/garden.ts` — чистая логика: пороги 9 стадий, множители улучшений, условия анлоков,
  `GARDEN_TESTERS` (гейт по Telegram id — сейчас только владелец).
- `src/pages/Garden.tsx` — полноэкранный под-вид вкладки «Прогресс» (ленивый чанк ~33 kB).
- `src/components/garden/Scene.tsx` — сцена слоями (небо → облака → дальний лес → ближний лес →
  остров → декор → дерево). Рисуется в логических координатах 360×272 и масштабируется
  `transform: scale` под ширину контейнера; всем `<img>` нужен `max-w-none`, иначе базовый reset
  ужимает спрайт и пиксельная сетка плывёт.
- `src/components/rewards/GardenTile.tsx` — плитка входа. У НЕ-тестера на этом месте прежняя
  `StreakTile`, поведение для обычных пользователей не изменилось.
- Улучшение «крепкие корни» — единственное, что трогает старую логику: `nextStreak`/`effectiveStreak`
  в `lib/streak.ts` получили параметр `shield` (серия переживает один пропущенный день).
- Персист: поле `garden`, **version 15**.
- **Графика:** `tools/build_assets.py` режет спрайты из `assets-raw/` (паки CraftPix) в
  `src/assets/game/`. `assets-raw/` в `.gitignore` — лицензия разрешает использовать графику в
  игре, но запрещает перераспространять сами паки. Регионы в атласах заданы явно: автопоиск
  связных областей не работает, объекты соприкасаются одиночными пикселями листвы.
