...
**Страницы:** `src/pages/` — Home, Analytics, Charts, **Profile**, Settings. Роутинг
табами в `src/App.tsx` (5 вкладок внизу, профиль — отдельная). Аватар в шапке Home
переключает на вкладку профиля (`onOpenProfile` → `setTab('profile')`).
В `App.tsx` смонтирован `DemoBanner` (плашка «Демо-режим · Выйти», видна когда `demoMode`).
...
## Текущее состояние
**Хэндофф:** проект в чистом состоянии. Последний деплой — бандл `index-B_oJfpdH.js`
(`APP_VERSION` = `1.13.0`). Воркер обновлён (добавлены `/data/get` и `/data/put`). Открытых багов нет.
