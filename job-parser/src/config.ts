...
export const CONFIG = {
  ...
  // 70 = fulltime + ещё хотя бы один плюс (ставка/близко/подходящий тип работы);
  // просто «fulltime без деталей» (60) не проходит.
  minScore: 70,

  // Сколько вакансий максимум слать за один прогон (остальное — строкой «и ещё N»).
  // 8, а не больше: бюджет подзапросов воркера (50) делится между источниками,
  // KV и отправкой двум подписчикам.
  maxPerRun: 8,
